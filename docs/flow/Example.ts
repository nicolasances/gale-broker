...
function buildingComputationTree() {
    ...
    start().branch("B1", [
        parallelize("group-section-classification", [agent(), agent(), agent()])
            .next().branch("B1.1", [
                parallelize("group-section-genealogy", [agent(), agent(), agent()]),
                parallelize("group-section-personalities", [agent(), agent(), agent()])
                    .next().agent()
            ]),
        parallelize("group-section-timeline", [agent(), agent(), agent()])
    ]).end()
